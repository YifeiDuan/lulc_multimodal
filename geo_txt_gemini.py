...
def batch_encode_rand_txt(img_dir="/content/drive/MyDrive/Courses/6.8300/Final Project/EuroSAT_MS", 
                          txt_dim=768,
                          mode="top", 
    ...
    if (mode == "sample") or (mode == "top"):
        save_pt_name = f"{mode}_{samples_per_class}_per_class.pt"
    save_dir = os.path.join(os.path.dirname(img_dir), "embeddings_rand_txt_gemini")
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    ...
    for idx, tif_path in tqdm(enumerate(all_tif_files), total=len(all_tif_files), desc="Encoding for: "):
        ...
        embedding = 0.01 * torch.randn(txt_dim)
